...
-- 1. View policy - simplified without recursion
CREATE POLICY "Users can view participants in their sessions" ON session_participants
FOR SELECT USING (
  auth.uid() = user_id
);

-- 2. Teachers can manage participants - check teacher status directly
CREATE POLICY "Teachers can manage participants" ON session_participants
FOR ALL USING (
  EXISTS (
